...
class SettingController
{
    ...
    private const KEY_LABELS = [
        ...
        'gateway.model_health_enabled' => [
            '坏模型自动暂停（推荐开着）',
            '开着：某个模型连续失败到阈值后**暂时**不再把请求发给它，用户再调它会立刻拿到'
            . '「该模型当前不可用」和可用的替代模型，而不是白等几分钟。'
            . '冷却到点自动放行一次试探，上游恢复后自动回到正常路由',
        ],
        'gateway.model_fail_threshold' => ['连续失败几次判定为坏', '只统计真正的模型级失败（连不上/超时、5xx、上游说没有这个模型）。参数错误与密钥问题不算'],
        'gateway.model_cooldown_seconds' => ['坏模型冷却秒数', '被暂停后过多久自动放行一次试探。设太短会反复撞墙，太长则上游恢复了也用不上'],
        'gateway.strip_unknown_params' => [
            '剥掉上游不认识的请求字段（推荐开着）',
            '开着：只把 OpenAI 标准字段（加上渠道自己声明的）发给上游，'
            . '客户端自带的私有参数不会害得整条请求被上游判 400。关掉则原样透传',
        ],
        'gateway.keep_body_params' => ['额外保留的请求字段名', '逗号分隔。某些上游支持非标准字段（各家自己的推理开关）时填这里'],
        'gateway.auto_strip_params' => [
            '自动学到的「上游不认的字段」',
            '由程序维护：上游在错误里点名说「不认识某字段」时，程序会把它记在这里，之后不再发送 —— '
            . '所以那个字段只会失败一次。清空本项表示「重新试探一遍」（上游升级支持了的话值得清一次）',
        ],
