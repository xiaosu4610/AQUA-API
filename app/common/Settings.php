...
final class Settings
{
     ...
     * 站点模式的中文名 —— **刻意返回空串**。
     *
     * 这里曾经返回「商业站 / 公益站」，并在页头渲染成一枚徽标。
     * 但那是一个**给用户看的划分标签**，副作用很实际：
     *   · 用户会以为站点被分成两类、功能有别（「公益站是不是不能充值？」）；
     *   · 正式对外运营时，标着「商业站」像在提醒顾客「你在被收钱」；
     *   · 而它对应的其实只是一个**站长自己的开关**（是否显示定价与充值入口）。
     *
     * 所以：开关保留（它确实控制功能可见性），但不再对外展示任何标签。
     * 返回值保留空串而不是删掉方法，是因为十几个模板都在用它渲染徽标 ——
     * 让它返回空串，徽标就在全站同时消失，不需要逐个模板去改。
     *
     * 各视图已按「空串就不渲染这个元素」处理，不会留下空壳徽标。
     */
